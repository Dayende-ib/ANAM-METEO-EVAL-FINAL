...
export function MonthlyMetricsContent() {
  ...
  if (loading) {
    return <LoadingPanel message="Chargement des métriques mensuelles..." />;
  }

  if (error) {
    return <ErrorPanel message={error} onRetry={loadMonthlyMetrics} />;
  }

  return (
    <div className="space-y-6">
      {/* Sélection du mois et recalcul */}
                  ...
                  <tbody>
                    {monthlyMetrics.map((metric, idx) => (
                      <tr
                        ...
                        className={`border-t border-[var(--border)] cursor-pointer hover:bg-[var(--surface-strong)] ${
                          selectedMetric && selectedMetric.year === metric.year && selectedMetric.month === metric.month 
