export type StatCardProps = {
  ...
  accent?: "primary" | "secondary" | "accent" | "gold" | "danger";
};
    ...
    <div className="surface-panel soft relative flex min-w-[160px] flex-1 flex-col gap-4 p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <p className="text-[11px] uppercase tracking-[0.32em] text-muted font-semibold">{label}</p>
          <p className="mt-2 text-sm font-normal text-ink font-mono leading-snug">
            {value}
          </p>
          {delta && <p className="mt-2 text-sm font-normal text-muted">{delta}</p>}
        </div>
