...
from backend.api_v1.core import _ensure_services_ready
...
async def list_data_issues(
    ...
    items = core.services.validation.list_data_issues(
        date=date,
    ...
    total = core.services.validation.count_data_issues(
        date=date,
        station_name=station,
        severity=severity,
        status=status,
...
async def get_quality_summary(date: Optional[str] = None):
    ...
    _ensure_services_ready()
    assert core.services is not None
    average = core.services.validation.get_average_quality_score(date=date)
    sample_size = core.services.validation.count_quality_scores(date=date)
...
async def ignore_issue(issue_id: int, payload: IssueStatusUpdate):
    ...
    _ensure_services_ready()
    assert core.services is not None
    core.services.validation.update_data_issue_status(issue_id, "ignored", payload.note)
...
async def correct_temperature(payload: TemperatureCorrectionRequest):
    ...
    _ensure_services_ready()
    assert core.services is not None
    updated = core.services.validation.update_temperatures_for_station(
        payload.date,
    ...
    if payload.issue_id:
        core.services.validation.update_data_issue_status(payload.issue_id, "fixed", None)
